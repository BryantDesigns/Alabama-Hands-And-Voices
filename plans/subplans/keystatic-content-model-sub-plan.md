...
- Use Keystatic local-file mode first.
...
- Add GitHub or Cloud editing later, after local schema and rendering are stable.
...
- [ ] Seed videos (deferred — no video data available yet).
...
- [ ] Keystatic admin loads all singletons and collections.
- [ ] Content edits write local files.
- [ ] Server reader can read every singleton.
- [ ] Server reader can list every collection.
- [x] Build passes with content files.
- [x] Next output tracing includes `src/content/**/*`.
- [ ] No route imports Firebase for content reads after migration.
